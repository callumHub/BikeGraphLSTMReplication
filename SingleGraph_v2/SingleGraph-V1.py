...
#########################################################################
# LSTM AutoEncoder
demandMaskTensor = tf.compat.v1.placeholder(tf.float32, [stationNumber, 1])
graphMatrixRaw = tf.compat.v1.placeholder(tf.float32, [stationNumber, 1])
graphMatrixWeight = tf.linalg.diag(tf.Variable(tf.random.normal([stationNumber])))
...
with tf.compat.v1.variable_scope('encoder'):
    ...
    en_outputs, en_final_state = tf.compat.v1.nn.dynamic_rnn(encoderCell, encoderInput, initial_state=encoderInitState,
                                                   time_major=False)
...
with tf.compat.v1.variable_scope('predict') as vs_p:
    ...
    regularization_cost = 0.001 * tf.reduce_sum(input_tensor=[tf.nn.l2_loss(v) for v in tv])
