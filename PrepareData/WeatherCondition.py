...
wind = {}
windHour = {}
with open(os.path.join(csvDataPath, '../new-york-weather-2015.csv'), 'r') as f:
    f_csv = csv.reader(f)
    headers = next(f_csv)
    counter = 0
    for row in f_csv:
        ...
        if row[2] != '':
            try:
                temperature[dateString].append(float(row[2].replace('s', '')))
                temperatureHour[dateString][hour] = float(row[2].replace('s', ''))
            except:
                print('Tem', row[2])
        #  风速
