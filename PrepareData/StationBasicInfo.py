...
os.system("pwd")
my_path = "../DataDir/NYC/csvData/"
assert os.path.exists(my_path)
csvFileNameList = [e for e in os.listdir(my_path) if e.endswith(".csv")]
#print(csvFileNameList, flush=True)
...
def task(ShareQueue, Locker, myCsvFileNameList, parameterList=None):
    stationAppearTime = {}
    for csvFile in myCsvFileNameList:
        with open(os.path.join(my_path, csvFile)) as f:
            f_csv = csv.reader(f)
            headers = next(f_csv)
            print(csvFile)
            for row in f_csv:
...
def without_mp(myCsvFileNameList):
    stationAppearTime = {}
    for csvFile in myCsvFileNameList:
        with open(os.path.join(my_path, csvFile)) as f:
            f_csv = csv.reader(f)
            ...
            for row in f_csv:
                ...
                if checkLatLng(startStationLat, startStationLong):
                    if startStationID not in stationAppearTime:
                        startStationName = row[4]
                        stationAppearTime[startStationID] = [startTime, startStationLat, startStationLong,
                                                             startStationName]
                    elif compareTime(startTime, stationAppearTime[startStationID][0]):
                        stationAppearTime[startStationID] = [startTime, startStationLat, startStationLong,
                                                             startStationName]
                if checkLatLng(endStationLat, endStationLong):
                    if endStationID not in stationAppearTime:
                        ...
                        stationAppearTime[endStationID] = [stopTime, endStationLat, endStationLong, endStationName]
    return stationAppearTime


if __name__ == '__main__':

    n_jobs = 39
    #csvFileNameList = [csvFileNameList[0]]
    ...
    #stationAppearTimeDict = without_mp(csvFileNameList)
