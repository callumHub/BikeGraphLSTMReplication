...
with autoEncoderGraph.as_default():
    ...
    with tf.compat.v1.variable_scope('encoder_fusionGraph'):
        # channel 1
        fusionGraphMatrixTensorRaw = tf.compat.v1.placeholder(tf.float32, [stationNumber, 1])
        fusionGraphMatrixWeight = tf.linalg.diag(tf.Variable(tf.random.normal([stationNumber])))
