...
with autoEncoderGraph.as_default():
    ...
    with tf.compat.v1.variable_scope('encoder_fusionGraph'):
        ...
        # channel 3
        corrGraphMatrixTensorRaw = tf.compat.v1.placeholder(tf.float32, [stationNumber, 1])
        corrGraphMatrixWeight = tf.Variable(tf.random.normal([stationNumber, 1]))
        ...
        encoderCell = tf.compat.v1.nn.rnn_cell.DropoutWrapper(encoderCell, output_keep_prob=1 - dropout_pro)
