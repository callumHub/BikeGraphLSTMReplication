...
with autoEncoderGraph.as_default():
    ...
    # LSTM AutoEncoder
    encoderRawInput = tf.compat.v1.placeholder(tf.float32, [batch_size, n_steps_encoder, stationNumber])
    decoderRawInput = tf.compat.v1.placeholder(tf.float32, [batch_size, n_steps_decoder, stationNumber])
    decoderRawTarget = tf.compat.v1.placeholder(tf.float32, [batch_size, stationNumber])
    demandMaskTensorAutoEncoder = tf.compat.v1.placeholder(tf.float32, [stationNumber, 1])

    # Demand Graph
    with tf.compat.v1.variable_scope('encoderDemandGraph'):
        demandGraphMatrixTensorRaw = tf.compat.v1.placeholder(tf.float32, [stationNumber, 1])
        demandGraphMatrixWeight = tf.linalg.diag(tf.Variable(tf.random.normal([stationNumber])))
        demandGraphMatrixTensor = tf.matmul(demandGraphMatrixWeight, demandGraphMatrixTensorRaw)

        encoderCell_DemandGraph = tf.compat.v1.nn.rnn_cell.LSTMCell(n_hidden_units, state_is_tuple=True)
        encoderCell_DemandGraph = tf.compat.v1.nn.rnn_cell.DropoutWrapper(encoderCell_DemandGraph,
        ...
        en_outputs_DemandGraph, en_final_state_DemandGraph = tf.compat.v1.nn.dynamic_rnn(encoderCell_DemandGraph, encoderInputDemandGraph,
                                                                               dtype=tf.float32,
                                                                               time_major=False)

    # Dist Graph
    with tf.compat.v1.variable_scope('encoderDistGraph'):
        distGraphMatrixTensorRaw = tf.compat.v1.placeholder(tf.float32, [stationNumber, 1])
        distGraphMatrixWeight = tf.linalg.diag(tf.Variable(tf.random.normal([stationNumber])))
        distGraphMatrixTensor = tf.matmul(distGraphMatrixWeight, distGraphMatrixTensorRaw)

        encoderCell_DistGraph = tf.compat.v1.nn.rnn_cell.LSTMCell(n_hidden_units, state_is_tuple=True)
        encoderCell_DistGraph = tf.compat.v1.nn.rnn_cell.DropoutWrapper(encoderCell_DistGraph, output_keep_prob=1 - dropout_pro)

        encoderInputDistGraph = tf.reshape(
            tf.matmul(tf.reshape(encoderRawInput, [-1, stationNumber]), distGraphMatrixTensor),
            [-1, n_steps_encoder, 1])

        en_outputs_DistGraph, en_final_state_DistGraph = tf.compat.v1.nn.dynamic_rnn(encoderCell_DistGraph, encoderInputDistGraph,
                                                                           dtype=tf.float32,
                                                                           time_major=False)
    ...
    fusionState = tf.nn.rnn_cell.LSTMStateTuple(fusionC, fusionH)
    ...
    with tf.compat.v1.variable_scope('decoder') as vs:
        ...
        decoderCell = tf.compat.v1.nn.rnn_cell.LSTMCell(n_hidden_units * 2, state_is_tuple=True)
        decoderCell = tf.compat.v1.nn.rnn_cell.DropoutWrapper(decoderCell, output_keep_prob=1 - dropout_pro)
        decoderOutputUnitNumber = n_hidden_units * 2
        decoderOutputWeight = tf.Variable(tf.random.normal([decoderOutputUnitNumber, n_inputs]), name='decoderOutputWeight')
        decoderOutputBias = tf.Variable(tf.constant(0.1, shape=[n_inputs, ]), name='encoderInputBias')
        decoderState = fusionState
        de_outputs, de_final_states = tf.compat.v1.nn.dynamic_rnn(decoderCell, decoderInput, initial_state=decoderState,
                                                        time_major=False)
    ...
    loss_autoEncoder = tf.reduce_mean(input_tensor=tf.square(decoderTarget - de_outputs))
    trainOperation_autoEncoder = tf.compat.v1.train.AdamOptimizer(lr).minimize(loss_autoEncoder)

    autoEncoderSaver = tf.compat.v1.train.Saver(max_to_keep=None)
    init_autoEncoder = tf.compat.v1.global_variables_initializer()

# prediction network
# fully connected
with preNNGraph.as_default():
    demandMaskTensorPreNN = tf.compat.v1.placeholder(tf.float32, [stationNumber, 1])
    inputEmbeddingFeature = tf.compat.v1.placeholder(tf.float32, [None, n_hidden_units*2])
    preNNInputRaw = tf.compat.v1.placeholder(tf.float32, [batch_size, stationNumber])
    preNNTargetRaw = tf.compat.v1.placeholder(tf.float32, [batch_size, stationNumber])
    preNNTarget = tf.matmul(preNNTargetRaw, demandMaskTensorPreNN)
    preNNInput = tf.matmul(preNNInputRaw, demandMaskTensorPreNN)
    otherFeature = tf.compat.v1.placeholder(tf.float32, [None, addFeatureLength])
    ...
    with tf.compat.v1.variable_scope('predict') as vs_p:
        W = [tf.Variable(tf.random.normal([n_units_pre[e], n_units_pre[e + 1]])) for e in range(n_units_pre.__len__() - 1)]
        ...
        for i in range(n_units_pre.__len__() - 2):
            ...
            preOutput = tf.nn.dropout(preOutput, rate=1 - (1 - dropout_pro))
    ...
    loss_pre = tf.reduce_mean(input_tensor=tf.square(preNNTarget - preOutput)) # + regularization_cost
    trainOperation_pre = tf.compat.v1.train.AdamOptimizer(lr).minimize(loss_pre)

    preSaver = tf.compat.v1.train.Saver(max_to_keep=None)
    init_preNN = tf.compat.v1.global_variables_initializer()
...
autoEncoderSession = tf.compat.v1.Session(config=config, graph=autoEncoderGraph)
...
preNNSession = tf.compat.v1.Session(config=config, graph=preNNGraph)
