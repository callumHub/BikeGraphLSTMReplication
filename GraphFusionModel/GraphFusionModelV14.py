...
n_epoch = 5001
batch_size = 128
lr = 0.00001  # learning rate (CHANGED: Divided from 0.0001 to 0.00001
n_hidden_units = 128 # OLD: 64 changed: 128
dropout_pro = 0.15 # OLD: Changed from 0.3 to 0.15
n_inputs = 1
n_steps_encoder = featureLength
n_steps_decoder = 3
decoderOutputSize = 1

timeRange = ['2015-01-01', '2015-11-30']
trainDataTimeRange = ['2015-01-01', '2015-11-30']
...
with autoEncoderGraph.as_default():
    ...
    with tf.compat.v1.variable_scope('encoder_fusionGraph'):
        # channel 1
        demandGraphMatrixTensorRaw = tf.compat.v1.placeholder(tf.float32, [stationNumber, 1])
        demandGraphMatrixWeight = tf.Variable(tf.random.normal([stationNumber, 1]))

        # channel 2
        distGraphMatrixTensorRaw = tf.compat.v1.placeholder(tf.float32, [stationNumber, 1])
        distGraphMatrixWeight = tf.Variable(tf.random.normal([stationNumber, 1]))
        ...
        # fusion graph
        fusionGraphMatrixWeight = tf.Variable(tf.random.normal([stationNumber, 1]))
        fusionGraphMatrixTensor = tf.multiply((demandGraphMatrixTensor + distGraphMatrixTensor), fusionGraphMatrixWeight)

        encoderCell = tf.compat.v1.nn.rnn_cell.LSTMCell(n_hidden_units, state_is_tuple=True)
        encoderCell = tf.compat.v1.nn.rnn_cell.DropoutWrapper(encoderCell, variational_recurrent=True,
                                                    dtype=tf.float32,
        ...
        en_outputs, en_final_state = tf.compat.v1.nn.dynamic_rnn(encoderCell, encoderInput, dtype=tf.float32, time_major=False)
    ...
    with tf.compat.v1.variable_scope('decoder') as vs:
        ...
        decoderCell = tf.compat.v1.nn.rnn_cell.LSTMCell(n_hidden_units, state_is_tuple=True)
        decoderCell = tf.compat.v1.nn.rnn_cell.DropoutWrapper(decoderCell, variational_recurrent=True,
                                                    dtype=tf.float32,
                                                    output_keep_prob=1 - dropout_pro, )
...
with preNNGraph.as_default():
    ...
    inputEmbeddingFeature = tf.compat.v1.placeholder(tf.float32, [None, n_hidden_units])
    ...
    with tf.compat.v1.variable_scope('predict') as vs_p:
        ...
        regularization_cost = 0.0001 * tf.reduce_sum(input_tensor=[tf.nn.l2_loss(v) for v in tv])
